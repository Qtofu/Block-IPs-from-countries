...
check_ipset
#添加ipset规则
echo -e "${Green}请输入禁止访问的国家代码，如cn(中国)，注意字母为小写！${Font}"
...
lookuplist=`ipset list | grep "Name:" | grep "$GEOIP"`
    if [ -n "$lookuplist" ]; then
        iptables -D OUTPUT -p tcp -m set --match-set "$GEOIP" src -j DROP
	iptables -D OUTPUT -p udp -m set --match-set "$GEOIP" src -j DROP
	ipset destroy $GEOIP
	echo -e "${Green}所禁止访问的国家($GEOIP)的ip解封成功，并删除其对应的规则！${Font}"
    else
...
clear
echo -e "———————————————————————————————————————"
echo -e "${Green}Linux VPS一键禁止访问指定国家所有的IP${Font}"
echo -e "${Green}1、封禁ip${Font}"
echo -e "${Green}2、解封iP${Font}"
echo -e "${Green}3、查看禁止访问列表${Font}"
echo -e "———————————————————————————————————————"
